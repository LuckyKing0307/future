...
@section('content')
    <!-- табы -->
    <div class="tab-bar">
        <button class="tab active" data-target="tab-in-process">{{ __('tasks.in_process_tab') }}</button>
        <button class="tab" data-target="tab-check">{{ __('tasks.check') }}</button>
        <button class="tab" data-target="tab-completed">{{ __('tasks.completed_tab') }}</button>
    </div>
